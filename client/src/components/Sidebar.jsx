...
const Sidebar = () => {
  ...
  return (
    <>
      <div
        className={`
        fixed inset-y-0 left-0 z-10 w-64 bg-gray-900 shadow-md overflow-hidden transition-transform duration-300
        ease-in-out
      ...
      >
        <div className='flex flex-col h-full'>
          {/* Header */}
          <div className='p-4 pb-[27px] border-b border-gray-800 flex justify-between items-center'>
            <h2 className='text-xl font-bold text-purple-400'>Matches</h2>
            <button
              className='lg:hidden p-1 text-gray-400 hover:text-gray-300 focus:outline-none'
          ...
          <div className='flex-grow overflow-y-auto p-4 z-10 relative'>
            {isLoadingMyMatches ? (
              <LoadingState />
            ) : (!matches || matches.length === 0) ? ( // Fix: Ensure matches exists
              <NoMatchesFound />
            ) : (
              matches.map((match) => (
                <Link key={match._id} to={`/chat/${match._id}`}>
                  <div className='flex items-center mb-4 cursor-pointer hover:bg-gray-800 p-2 rounded-lg transition-colors duration-300'>
                    <img
                      ...
                      alt='User avatar'
                      className='size-12 object-cover rounded-full mr-3 border-2 border-purple-500'
                    />

                    <h3 className='font-semibold text-gray-300'>{match.name}</h3>
                  </div>
                </Link>
              ))
            )}
      ...
      <button
        className='lg:hidden fixed top-4 left-4 p-2 bg-purple-700 text-white rounded-md z-0'
        onClick={toggleSidebar}
      >
